-- ============================================
-- CORRIGIR FUNÇÃO - VERSÃO FINAL
-- ============================================
-- Usa UPDATE sem cast explícito, deixando PostgreSQL fazer conversão
...
BEGIN
  ...
  -- CORREÇÃO: Usar EXECUTE com SQL dinâmico para evitar problemas de cast
  -- Mapear para valores válidos do ENUM
  CASE v_plan_type
    WHEN 'academy_starter' THEN v_plan_type := 'academy_starter';
    WHEN 'academy_growth' THEN v_plan_type := 'academy_growth';
    WHEN 'personal_team' THEN v_plan_type := 'personal_team';
    WHEN 'monthly' THEN v_plan_type := 'monthly';
    WHEN 'annual' THEN v_plan_type := 'annual';
    ELSE v_plan_type := 'free';
  END CASE;
  
  -- Construir SQL dinâmico
  v_update_sql := format(
    'UPDATE user_profiles SET plan_type = %L::plan_type, subscription_status = %L, account_type = %L, updated_at = now() WHERE id = %L',
    v_plan_type,
    'active',
