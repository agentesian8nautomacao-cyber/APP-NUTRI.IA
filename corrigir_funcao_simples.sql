-- ============================================
-- CORRIGIR FUNÇÃO - VERSÃO SIMPLES
-- ============================================
-- Usa UPDATE direto com string literal
...
BEGIN
  ...
  -- CORREÇÃO: Usar CASE para mapear para valores válidos e fazer UPDATE com string literal
  CASE v_plan_type
    WHEN 'academy_starter' THEN
      UPDATE user_profiles
      SET plan_type = 'academy_starter'::plan_type,
          subscription_status = 'active',
          account_type = COALESCE(v_account_type, account_type, 'USER_B2C'),
          updated_at = now()
      WHERE id = p_user_id;
    WHEN 'academy_growth' THEN
      UPDATE user_profiles
      SET plan_type = 'academy_growth'::plan_type,
          subscription_status = 'active',
          account_type = COALESCE(v_account_type, account_type, 'USER_B2C'),
          updated_at = now()
      WHERE id = p_user_id;
    WHEN 'personal_team' THEN
      UPDATE user_profiles
      SET plan_type = 'personal_team'::plan_type,
          subscription_status = 'active',
          account_type = COALESCE(v_account_type, account_type, 'USER_B2C'),
          updated_at = now()
      WHERE id = p_user_id;
    WHEN 'monthly' THEN
      UPDATE user_profiles
      SET plan_type = 'monthly'::plan_type,
          subscription_status = 'active',
          account_type = COALESCE(v_account_type, account_type, 'USER_B2C'),
          updated_at = now()
      WHERE id = p_user_id;
    WHEN 'annual' THEN
      UPDATE user_profiles
      SET plan_type = 'annual'::plan_type,
          subscription_status = 'active',
          account_type = COALESCE(v_account_type, account_type, 'USER_B2C'),
          updated_at = now()
      WHERE id = p_user_id;
